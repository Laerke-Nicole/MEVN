<template>
  <div class="bg-[#181818] min-h-screen lazy-css">
    ...
    <div class="text-center" v-if="loading">Loading...</div>                             <!-- Loading wait screen -->
    <div class="text-center text-red-500" v-else-if="error">{{ error }} </div>                         <!-- Error message -->
    <div class="flex flex-wrap -mx-2" v-else>                                    <!-- add new product section -->
    ...
    <div class="my-8 p-2 w-full">
      ...
      <div class="mb-4 p-4 border rounded bg-[#181818]" v-for="product in products" :key="product._id"> <!-- Loop through the products -->
         <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <span>
            Thumbnail img: <img alt="Product Image" :src="product.imageURL" class="w-full h-24 w-24 object-cover mb-4 rounded-lg"> <!-- Product image -->
          </span>
        </div>
...
const addProductHandler = async () => {
  // import userId from the useproducts
